...
{
//    NSMutableData * webData = [data mutableCopy];
    
//    NSString *theXML = [[NSString alloc] initWithBytes: [webData mutableBytes] length:[webData length] encoding:NSUTF8StringEncoding];
    XMLReader *reader = [[XMLReader alloc] initWithError:error];
    NSMutableDictionary *rootDictionary = [[reader objectWithData:data options:0] mutableCopy];
    rootDictionary = [self extractXML:rootDictionary];
    
    if (key.length > 0) {
        rootDictionary = [self findValueForKey:key inDictionary:rootDictionary];        
    }
    return rootDictionary;
}

+ (id)findValueForKey:(NSString*)_key inDictionary:(NSDictionary*)XMLDictionary
{
    static id dict = nil;
    for (NSString *key in [XMLDictionary allKeys]) {
        ...
        if ([object isKindOfClass:[NSDictionary class]]) {
            if ([object allKeys].count > 0) {
                if ([[[object allKeys] objectAtIndex:0] isEqualToString:_key] &&
                    [[object objectForKey:_key] isKindOfClass:[NSDictionary class]]) {
                    // this means the object has the key "text" and has no node
                    // or array (for multiple values) attached to it.
                    dict = object;
                    return dict;
                }
                [self findValueForKey:_key inDictionary:object];
                //[XMLDictionary setObject:[object objectForKey:@"DashboardResult"] forKey:key];
            }
            else {
                // go deeper
            ...
            for (id inArrayObject in (NSArray *)object) {
                if ([inArrayObject isKindOfClass:[NSDictionary class]]) {
                    ...
                    [self findValueForKey:_key inDictionary:inArrayObject];
                }
            }
        }
    }
...
+ (NSDictionary *)dictionaryForXMLString:(NSString *)string valueForKey:(NSString*)key error:(NSError **)error
{
    ...
    return [XMLReader dictionaryForXMLData:data valueForKey:key error:error];
}
