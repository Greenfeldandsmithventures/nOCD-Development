...
- (void)setXLabels:(NSArray *)xLabels withWidth:(CGFloat)width
{
    ...
    if (_showLabel) {
        for (int index = 0; index < xLabels.count; index++) {
            ...
            NSInteger x = self.chartLeftMargin +  (index * _xLabelWidth) - (_xLabelWidth / 2);
            NSInteger y = self.frame.size.height -(_chartMargin +  _chartBottomMargin);
...
- (void)calculateChartPath:(NSMutableArray *)chartPath andPointsPath:(NSMutableArray *)pointsPath andPathKeyPoints:(NSMutableArray *)pathPoints andPathStartEndPoints:(NSMutableArray *)pointsOfPath
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            int x = self.chartLeftMargin +  (i * offSetX);
            int y = _chartCavanHeight - (innerGrade * _chartCavanHeight) + (_yLabelHeight / 2) + _chartTopMargin;
...
- (void)setChartData:(NSArray *)data
{
    if (data != _chartData) {
        ...
        for (PNLineChartData *chartData in data) {
            ...
            pointLayer.strokeColor   = [[chartData.strokeColor colorWithAlphaComponent:chartData.alpha]CGColor];
            pointLayer.lineCap       = kCALineCapRound;
            pointLayer.lineJoin      = kCALineJoinBevel;
            pointLayer.fillColor     = [[chartData.pointColor colorWithAlphaComponent:chartData.alpha]CGColor];
...
- (void)setupDefaultValues
{
    ...
    _chartMargin = 40.0f;
    _chartLeftMargin = 25.0f*deviceScale;
    _chartBottomMargin = 30.0f*deviceScale;
    _chartTopMargin = 10.0f*deviceScale;
    if (!IS_IPHONE_5) {
        _chartTopMargin = 40.0f*deviceScale;
    }
